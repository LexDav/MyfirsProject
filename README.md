...
## Шаг 6. Прогон проверок перед ручным тестированием
...
> В репозитории также добавлен CI workflow: `.github/workflows/python-ci.yml` (повторяет compile-check и `pytest`).

---

## Готовность к тестовому прогону (helicopter-view)

Текущее состояние проекта — **готов к тестовому прогону (MVP QA-ready)** при соблюдении базовых предусловий:

1. Загружены коды в `bot.db` через `import_codes.py`.
2. Установлен корректный `BOT_TOKEN`.
3. Для LLM-сценариев задан `OPENAI_API_KEY` (иначе бот работает в fallback-режиме по ключевым словам).

Что уже усилено перед тестированием:
- Режим пользователя (Light/Expert) сохраняется в БД и восстанавливается после перезапуска.
- Добавлены smoke-тесты и CI-проверка.
- Усилен парсинг/валидация LLM-ответа и fallback при ошибках.
...
## Инфраструктура проверок в репозитории

> Эти файлы находятся в корне репозитория (не внутри папки `TG Bot VED/`):

- `.github/workflows/python-ci.yml` — CI-пайплайн (`py_compile` + `pytest`)
- `tests/test_bot_core.py` — smoke/unit тесты критичных функций
