...
def contains_valid_tnved_code(text: str) -> bool:
    """Проверяет, есть ли в тексте валидный 10-значный код ТН ВЭД (группы 01-97)."""
    raw = (text or "").strip()
    if raw.isdigit() and len(raw) == 10:
        return 1 <= int(raw[:2]) <= 97

    m = CODE10_RE.search(raw)
    if not m:
        return False

    return 1 <= int(m.group(1)[:2]) <= 97
...
def tnved_search_candidates(db_path: str, query_text: str, limit: int = 12) -> list[tuple[str, str]]:
    ...
    # Подсчёт релевантности: чем больше совпавших токенов, тем выше позиция.
    top_tokens = tokens[:6]
    score_expr = " + ".join([f"CASE WHEN title LIKE ? THEN 1 ELSE 0 END" for _ in top_tokens])
    where = " OR ".join(["title LIKE ?"] * len(top_tokens))
    token_params = [f"%{t}%" for t in top_tokens]
    sql = f"""
        SELECT code, title, ({score_expr}) AS score
        FROM tnved
        WHERE {where}
        ORDER BY score DESC, code ASC
        LIMIT ?
    """
    cur.execute(sql, (*token_params, *token_params, limit))
...
async def classify_command(message: Message, state: FSMContext, command: CommandObject) -> None:
    ...
    if command.args:
        ...
        if not contains_valid_tnved_code(text) and not assess_expert_input(text):
            await message.answer(
